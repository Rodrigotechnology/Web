	...
	class WpAutomaticFacebook extends wp_automatic {
		...
		function fb_get_post($camp) {
			...
			if ((wp_automatic_trim( $cg_fb_page_id ) != '')) {
				...
				if( stristr($exec,'page_uri:"https://m.facebook.com/login.php') ){
				...
				}elseif ((stristr ( $exec, 'payload' ) && stristr ( $exec, $cg_fb_page_id ) && ! stristr($exec,'page_uri:"https://m.facebook.com/login.php') ) || (stristr ( $exec, 'notifications.php' ) && ($cg_fb_from == 'events'  || $cg_fb_source == 'group') )) { // Checks that the object is created correctly
					// correct login
